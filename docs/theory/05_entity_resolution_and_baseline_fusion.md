...
## 5.9. DBSCAN с cost-optimal выбором параметров

Проблема J1: транзитивное замыкание DSU дало 49 кластеров, purity 0.000 (пере-слияние при address $\approx$ txId). DSU не различает ложные связи, порождаемые CIOH на биржевых hot wallets и CoinJoin — транзитивность «все с каждым» внутри транзакции растворяет границы субъектов.

**DBSCAN на эмбеддингах** решает это через локальную плотность вместо глобальной транзитивности. Параметры подбираются не эвристически, а через grid search по cost-функции:

- `eps` — квантиль k-distance графика (k = min_pts), сетка квантилей: $\{0.80, 0.85, 0.90, 0.95, 0.99\}$
- `min_pts` — сетка: $\{5, 10, 20, 50, 100\}$
- `max_size` — size-cap: кластеры выше порога рекурсивно разбиваются по плотности (DBSCAN с меньшим eps); если плотность не позволяет — KMeans fallback с $k = \lceil \text{size} / \text{max\_size} \rceil$

**Выбор параметров:** grid search (eps quantiles $\times$ min_pts) по минимуму
$$
\text{cost} = C_{FP} \cdot FP + C_{FN} \cdot FN
$$
на валидации (walk-forward temporal split). Здесь $FP$ — ложное слияние (две сущности в одном кластере), $FN$ — ложный split (одна сущность в разных кластерах). Парные метрики считаются на ground truth Elliptic++: для каждого кластера $C$ ищется доминирующая сущность $e^* = \arg\max_e |C \cap e|$; $FP = |C| - |C \cap e^*|$, $FN = \sum_{e \neq e^*} |C \cap e|$. Коэффициенты $C_{FP}, C_{FN}$ берутся из cost-функции операционной точки (раздел 8), базовое соотношение $C_{FN}/C_{FP} = 10$.

**Результат:** честный negative result vs J1 (purity $\le$ baseline) или purity $> 0.6$ — оба допустимы по DoD. Negative result означает, что на текущих эмбеддингах DBSCAN не обыгрывает транзитивное слияние — это валидный вывод, исключающий дальнейшие попытки без улучшения представлений. Purity $> 0.6$ — сигнал, что локальная плотность в эмбеддингах лучше разделяет субъекты, и DBSCAN становится дефолтом вместо DSU.
