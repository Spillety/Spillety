...
Риск-скор без проверяемого обоснования не удовлетворяет требованиям FinCEN, FATF и SR 26-2 и не обладает court-admissibility. Раздел описывает структуру evidence JSON как проверяемого паспорта алерта, механизм provenance и гарантии неизменяемости на основе WORM-хранения, Ed25519-подписей, Merkle-дерева и привязки к блокчейну через OpenTimestamps.
...
1. **Регуляторное соответствие.** FinCEN, FATF, 6AMLD и SR 26-2 требуют документированного обоснования каждого алерта.
...
Аналитик видит, какие anchors сформировали оценку, и может верифицировать каждый независимо.
...
Контекст поведения кошелька для сопоставления с типичными значениями класса.
...
Вклады признаков в риск-скор; необходимы для интерпретируемости и human review.
...
Provenance обеспечивает **воспроизводимость**: зная версии encoder'а, GBDT, HNSW, калибратора и $\tau$, а также источник данных, независимый валидатор повторяет вычисления и получает идентичный risk_score.
...
WORM (Write Once Read Many) — хранение, при котором данные после фиксации не могут быть изменены или удалены. Для court-admissibility: любая ретроспективная модификация алерта детектируется криптографически.

WORM реализуется тремя механизмами:
...
Любое изменение одного байта JSON меняет $h'$ и верификация не проходит. Тест: модификация $1$ байта в $1\text{M}$ алертов — $100\%$ детекции; latency верификации $<0.1$ мс.
...
1. Хеши алертов за период объединяются в дерево.
2. Корень $R$ публикуется и подписывается $sk$.
3. Для каждого алерта формируется `merkle_proof`.

**Верификация Merkle proof:** по листу $h_i$ и sibling-хешам восстанавливается кандидат $R'$; проверка $R' \stackrel{?}{=} R_{\text{published}}$. Не требует раскрытия остальных $N-1$ алертов. Тест: верификация $1\text{M}$ proofs — p95 latency $<5$ мс, recall принадлежности $1.0$.
...
Evidence JSON фиксирует версию калибратора GBDT. Выбор **isotonic vs beta** влияет на достоверность `risk_score` и `shap_values`:
...
Audit trail хранится в WORM-хранилище; записи неизменяемы. Все взаимодействия с алертом — только через систему.
...
SAR-поток зависит от порога $\tau$:
...
- **95-й vs 99-й квантиль:** влияет на частоту retrain и косвенно на стабильность SAR-потока.
...
|-------|--------------|-------|----------------|-------------------|
| Дрейф-детектор | Quantile vs cost-function | Recall дрейф-событий, expected cost, latency | Робастность без $C_{FP}$/$C_{FN}$ vs минимум expected cost | Quantile для детекции дрейфа; cost для $\tau$ алертов |
...
- **KS $p+D$+Cohen $d$ + silhouette** — различают drift/новый паттерн/шум.
- **PR-AUC walk-forward** — оценка обобщения encoder'а, версия фиксируется в provenance.
- **Brier/ECE drift** — контроль калибровки `risk_score`.
- **recall@K** — качество HNSW, версия фиксируется в provenance.
- **power $n$** — достаточность random sampling для unbiased precision/recall.
- **audit verification / Merkle proof** — проверяемость каждого алерта независимо от хранилища.

Все выборы версионируются в provenance и проверяются детерминированно.
...
**Merkle tree:** Дерево хешей для 8 алертов. Листья — $\text{SHA-256}$ алертов, корень — $R$, привязываемый к Bitcoin через OpenTimestamps. Merkle proof — путь от листа к корню.

![audit trail](./files/9-5-2_audit_trail.png)

**Audit trail:** Хронология действий с алертом. Цвет — тип действия; каждая запись — WORM, проверяется через audit verification.
...
2. **Key management Ed25519:** компрометация $sk$ компрометирует все подписи; требуется HSM, ротация ключей и публикация $pk$.
...
4. **Размер Merkle proof:** $\log_2 N$ хешей ($20$ для $1\text{M}$, $30$ для $1\text{B}$) — логарифмический рост.
5. **Полнота audit trail:** взаимодействия вне системы (email, устно) не фиксируются; требование — все действия только через систему.
6. **SAR throughput:** human review обязателен, ограничивает пропускную способность.
...
8. **Зависимость от Brier/ECE:** неадекватная калибровка делает `risk_score` проверяемым, но семантически ненадёжным — требуется drift-мониторинг.
