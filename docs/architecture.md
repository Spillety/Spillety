...
В волне L1 в `sage.py` добавлен `encode_gated` — feature-gated GraphSAGE, который решает проблему
конкуренции между raw-признаками и агрегированными данными на каждом слое.
Вместо простого суммирования берётся residual pathway для сырых признаков,
а между ним и агрегированным вектором ставится обучаемый gate:
`g = sigmoid(W_r @ x_raw + W_a @ x_agg + b)`,
`out = g * proj(x_raw) + (1-g) * x_agg`.
Gate позволяет модели на ранних слоях фокусироваться на локальных признаках,
а на поздних — на графовой структуре. Тесты в `test_embeddings` проверяют,
что форма выхода сохраняется и градиенты текут через gate.

### Decoupled temporal encoder

В волне L3 в `temporal.py` добавлен `DecoupledEvolveGCNEncoder`,
который разделяет GRU (для темпоральных признаков) и GCN (для графовой структуры)
с gated fusion между ними. Это устраняет optimization conflict: исходный EvolveGCN
смешивает оба типа информации в одном обновлении весов, и GRU конкурирует
с GCN за одно и то же пространство. Decoupled версия даёт каждому механизму
своё поле ответственности. На GBDT-скоринге без torch_geometric gain нет —
gain появляется только при end-to-end обучении на графе.
...
Финальный ансамбль `models/elliptic_v3` держит PR-AUC 0.656 на тестовых шагах
...
### Wavelet-расширение признаков (эксперимент)

В волне L2 в `features/wavelet.py` добавлен Haar level-2 wavelet (`haar_level2`), который
удваивает число признаков с 165 до 342 за счёт деталей на двух масштабах.
Логика: temporal-признаки в Elliptic++ уже кодируют динамику,
и wavelet должен выделить high-freq компоненты (резкие всплески активности),
которые GBDT не ловит на сырых данных. Результат на `models/elliptic_v4/` — PR-AUC 0.627
против 0.656 у v3: wavelet добавляет шум, а не сигнал. Причина в том, что 165 исходных признаков
уже покрывают temporal structure, и дополнительная декомпозиция дублирует информацию.
Эксперимент зафиксирован, но wavelet не включён в продакшен-пайплайн.

### Focal loss (эксперимент)

В волне K4 в `models/focal.py` реализован focal loss objective для LightGBM.
Фокус на hard-to-classify примерах через модуль `γ(1-p_t)^γ` в бинарной кросс-энтропии.
На v4 + wavelet PR-AUC упал до 0.199 — числовая нестабильность при large logits,
clip [-30,30] не спасает. Focal loss требует более тонкой настройки гиперпараметров
(γ, learning rate, scale_pos_weight) и не рекомендуется к использованию без экспериментов.
