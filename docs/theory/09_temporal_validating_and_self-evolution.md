...
## 9.3.5. Lead time: скорость реакции на новые санкции

Lead time — уникальная метрика самопроверки: время от появления адреса в санкционном списке до первого алерта по нему в топ-K.

$$LT(a) = t_{\text{first\_alert}}(a) - t_{\text{sanction}}(a)$$

где $t_{\text{first\_alert}}(a)$ — первый шаг, когда $a$ попал в топ-K по скору (прошедший DAG-фильтр и cost-based $\tau^*$), а $t_{\text{sanction}}(a)$ — шаг добавления адреса в OFAC/EU список.

Метрики:
- **Медианный lead time** (цель: $\le 3$ шагов)
- **P90 lead time**
- **Стратификация по эпохам** (до/после шага 43 — смена режима в Elliptic++)
- **Censored anchors**: якори, не попадавшие в топ-K до санкции — исключаются из медианы, считаются отдельно
- **Recall@K на новых санкциях**: доля post-train якорей с алертом в топ-K до момента санкции

Lead time — не просто метрика качества, а **self-evolution signal**: растущая медиана lead time → триггер переобучения (помимо дрейф-гейта). Это создаёт замкнутый контур: качество измеряется не на статичном тесте, а на потоке реальных санкций.
...
| Lead time | — | Цель $\le 3$ шага, рост → триггер переобучения | Lead time в self-evolution контуре |
