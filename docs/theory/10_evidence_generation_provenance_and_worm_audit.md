...
| anchors | array | Якоря с полями wallet/source/distance/causal_filter/e_value/rosenbaum_gamma/sensemakr_r2 — **ТОЛЬКО это поле входит в Merkle/OTS подпись** (доказуемое утверждение) |
| causal_path | array | Причинные рёбра и оценки эффекта — производное поле, **не в подписи** |
| graph_features | object | Структурные и темпоральные признаки |
| shap_values | object | Вклады признаков (SHAP, раздел 7) |
| provenance | object | Версии моделей (encoder/gbdt/calibration), HNSW params, threshold tau, cost_ratio — производное поле, **не в подписи** |
| audit | object | Подпись, Merkle proof, метки времени |

**Важно:** В Merkle/OTS подпись входит ТОЛЬКО `anchors` (доказуемое утверждение о близости к санкционным адресам). Поля `causal_path`, `provenance`, `shap_values`, `graph_features`, `gigachat_narrative` (если присутствует) — производные, не в подписи. Это разделение критично для аудита: верификатор проверяет подпись над anchors, затем независимо воспроизводит производные поля из версий в provenance.
...
### 10.2.7. GigaChat-объяснение (derived поле narrative)

GigaChat-объяснение — derived поле `narrative` в SAR draft, вне signed canonical evidence JSON. Протокол вызова:

1. **verify-first**: перед вызовом LLM — полная верификация evidence (подпись Ed25519, Merkle proof, корень, OTS). Только после pass — контекст уходит в GigaChat.
2. **post-validate**: ответ GigaChat валидируется — все адреса/суммы в narrative должны находиться в контексте evidence (anchors, causal_path, graph_features). Галюцинации, вводящие новые сущности, — reject и retry.
3. **human-gate**: поле `narrative` пишется только в `sar.narrative` при `status=draft`; подача SAR (`approve_sar`) требует `reviewer_id`; автоподача запрещена кодом (`PermissionError`).

Это гарантирует, что LLM-вывод никогда не становится частью криптографически подписанного артефакта и не может повлиять на верифицируемую цепочку доказательств.
...
**Реализация:** async-клиент `ots_anchor_async` (submit к календарю OTS) + `ots_verify` (верификация proof против Bitcoin). Polling/подтверждение — вызывающая сторона; stub `ots_anchor` остаётся с ponytail до прод-ключей. Это разделение позволяет тестировать контур без живого Bitcoin и контролировать латентность через таймауты вызывающего кода.
